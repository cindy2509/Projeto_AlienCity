...
public static class CMD {
...
#if !CMDPLUS_EXCLUDED_IN_BUILD
        if(!Application.isPlaying || Initialized || initializing) { return; }
        initializing = true;
    ...
    public static CMDLogEntry ExecuteCommand (string commandLine) {
        ...
        if (splitComandLine.Length == 0) { return null; }
...
namespace CMDPlus {
    public struct CMDVersion {
        ...
        public override string ToString() {
            ...
            if(Build >= 0) { result += string.Format(".{0}", Build); }
            if (Revision >= 0) { result += string.Format(".{0}", Revision); }
            return result;
        }
    }
}